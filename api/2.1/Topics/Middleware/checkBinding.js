...
module.exports = async (req, res, next) => {
  ...
  Extensions = conn.model(
    "Extensions",
    require("../../Projects/Models/extensions")
  );
  module.exports = conn;
          ...
          if (!topic_type.includes(req.body["topic_type"])) {
            return res.status(400).json({
              Message:
                'Could not find "' +
                req.body["topic_type"] +
                '" in the projects type extensions',
            });
          }
        }
        ...
        if (req.body["topic_status"]) {
          if (!topic_status.includes(req.body["topic_status"])) {
            return res.status(400).json({
              Message:
                'Could not find "' +
                req.body["topic_status"] +
                '" in the projects status extensions',
            });
          }
        }
          ...
          if (!priority.includes(req.body["priority"])) {
            return res.status(400).json({
              Message:
                'Could not find "' +
                req.body["priority"] +
                '" in the projects priority extensions',
            });
          }
        }
          ...
          if (!user_id_type.includes(req.body["assigned_to"])) {
            return res.status(400).json({
              Message:
                ...
                '" in the project user extensions',
            });
          }
        }
          ...
          if (!stage.includes(req.body["stage"])) {
            return res.status(400).json({
              Message:
                'Could not find "' +
                req.body["stage"] +
                '" in the projects stage extensions',
            });
          }
        }
